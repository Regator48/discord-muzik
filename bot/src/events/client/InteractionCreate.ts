...
export default class InteractionCreate extends Event {
	...
	public async run(interaction: Interaction<CacheType>): Promise<any> {
		...
		if (
			...
			let component = this.client.components.get((interaction as any).customId);
			
			// Support prefix matching for components (e.g., pdf_dup_keep_*)
			if (!component) {
				const customId = (interaction as any).customId;
				logger.info(`[DEBUG] Button interaction received: ${customId}`);
				logger.info(`[DEBUG] Available components: ${Array.from(this.client.components.keys()).join(', ')}`);
				for (const [name, comp] of this.client.components) {
					if (customId.startsWith(name)) {
						logger.info(`[DEBUG] Matched component: ${name}`);
						component = comp;
						break;
					}
				}
				if (!component) {
					logger.warn(`[DEBUG] No component matched for: ${customId}`);
				}
			}
			...
			if (component) {
				try {
					...
					logger.error(`[DEBUG] Component error: ${error}`);
				}
			} else {
				// Acknowledge the interaction to prevent "application didn't respond"
