...
export default class TrackEnd extends Event {
	...
	public async run(player: Player, _track: Track | null, _payload: TrackStartEvent): Promise<void> {
		...
		// Don't delete message immediately - let TrackStart recreate it
		// Instead, just update the message with next track info
		try {
			await channel.messages.fetch(messageId).catch(() => null);
		} catch {
			// Message may already be deleted, that's fine
		}
		// Don't delete - TrackStart will handle the new message
	}
}
