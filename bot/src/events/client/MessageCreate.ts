...
export default class MessageCreate extends Event {
	...
	public async run(message: Message): Promise<any> {
		...
		// Wallahi auto-response
		if (message.content.trim().toLowerCase() === "wallahi") {
			const responses = [
				"good goy 😊",
				"+7k from israel gov 💰",
				"zion dong 🐀",
			...
			await message.reply({ content: responses[Math.floor(Math.random() * responses.length)], flags: MessageFlags.SuppressNotifications });
		}
