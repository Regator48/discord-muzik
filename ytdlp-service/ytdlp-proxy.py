...
class YtdlpHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if self.path.startswith("/resolve"):
                ...
                if url.startswith("ytsearch"):
                    ...
                    result = subprocess.run(
                        [YTDLP, "--no-warnings", "-f", "bestaudio", "-g",
                         "--match-filter", "!is_live & duration>30",
            ...
            # Use yt-dlp to stream raw audio directly (no ffmpeg transcoding = faster startup)
            dl = None
            try:
                dl = subprocess.Popen(
                ...
                # Stream directly - raw WebM/Opus from yt-dlp
...
if __name__ == "__main__":
    ...
    server = ReuseAddrThreadingHTTPServer(("127.0.0.1", 4567), YtdlpHandler)
    print("yt-dlp proxy on :4567 (threaded)", file=sys.stderr)
