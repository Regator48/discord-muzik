...
export default class Context {
	...
	/**
	 * Prepends the Discord silent-message flag to content (suppresses push notifications).
	 */
	private silentContent(
		content: string | MessagePayload | MessageCreateOptions | InteractionReplyOptions,
	): string | MessagePayload | MessageCreateOptions | InteractionReplyOptions {
		if (typeof content === "string") {
			const prefix = content.startsWith(SILENT_PREFIX) ? "" : `${SILENT_PREFIX} `;
			return { content: `${prefix}${content}`, flags: MessageFlags.SuppressNotifications } as MessageCreateOptions;
		}
		if (content instanceof Object) {
			const opts = content as {
				content?: string;
				flags?: number | bigint;
			};
			if ((opts.flags ?? 0) !== 0) {
			...
			if (typeof opts.content === "string" && !opts.content.startsWith(SILENT_PREFIX)) {
				opts.content = `${SILENT_PREFIX} ${opts.content}`;
			}
			return opts;
		}
		return content;
	...
	public async sendMessage(
		...
		if (this.isInteraction) {
			if (typeof content === "string" || isInteractionReplyOptions(content)) {
				this.msg = await this.interaction?.reply(this.silentContent(content) as InteractionReplyOptions);
				return this.msg;
			}
	...
	public async editMessage(
		...
		if (this.msg) {
			this.msg = await this.msg.edit(this.silentContent(content) as MessageEditOptions);
			return this.msg;
		}
		return this.msg;
	}
	...
	public async sendDeferMessage(
		...
		if (this.isInteraction) {
			if (this.interaction && !this.interaction.deferred && !this.interaction.replied) {
				await this.interaction.deferReply({ flags: MessageFlags.SuppressNotifications });
			}
			this.msg = (await this.interaction?.editReply(
				this.silentContent(content) as string | MessagePayload | WebhookMessageEditOptions,
			)) as Message;
			return this.msg;
		}

		this.msg = await (this.message?.channel as TextChannel).send(this.silentContent(content) as string | MessagePayload);
		return this.msg;
	}
